let Town = {
    ...
    renderShop: function() {
        ...
        for (let i = 0; i < this.shopItems.length; i++) {
            let canBuy = true;
            let own = false;
            if (player.pokeCoins < this.shopItems[i].cost)
                canBuy = false;
    ...
    buyItem: function(index) {
        ...
        if (player.pokeCoins < item.cost) {
            return false;
        } else {
            player.pokeCoins -= item.cost;
            if (item.ball) {
