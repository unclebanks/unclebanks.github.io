...
export default (player, combatLoop, enemy, town, story, appModel) => {
    ...
    const UserActions = {
        ...
        renderGym: function() {
            ...
            for (let i = 1; i < 4; i++) {
                $(`#gymTrainer${i}`).style.display = (route.gym.gymTrainer1) ? '' : 'none';
                $(`#gymTrainer${i}`).innerHTML = (route.gym.gymTrainer[i - 1]) ? route.gym.gymTrainer[i - 1].name : '';
            }
        ...
        gymTrainerBattle: function() {
            const routeData = ROUTES[player.settings.currentRegionId][player.settings.currentRouteId].gym;
            closeModal(document.getElementById('gymModal'));
            for (let i = 1; i < 4; i++) {
                if (routeData.gymTrainer[i - 1].req && !player.wins[routeData.gymTrainer[i - 1].req]) {
                    notify('Defeat more GYM LEADERS and try again');
                } else if (routeData.gymTrainer[i - 1] && routeData.gymTrainer[i - 1].poke.length > 0) {
                    combatLoop.gymLeader = { name: routeData.gymTrainer[i - 1].name, win: routeData.gymTrainer[i - 1].win };
                    combatLoop.gymLeaderPoke = Object.values({...routeData.gymTrainer[i - 1].poke });
                    combatLoop.unpause();
                    combatLoop.refresh();
